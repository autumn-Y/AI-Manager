...
def uv_data():
    ...
    servicekey = 'XZ9kxxXefiUI3gSeGe0/RtOtOSjRP+7pPEiUe6kYZmip/MAQkHJCLmEySAUSOnQYihwtcMF+GRAexUoiGaw+nA=='
    url = (url + '?' + servicekey +
           '&pageNo=1&numOfRows=1&dataType=XML&areaNo=1100000000' +
           '&time=' + base_date + '05')
    ...
    uv_df = pd.DataFrame(row_list, columns=name_list)
    uv_df = uv_df.T
    uv_df = uv_df.iloc[3: 11]
